...
export function InventoryStockMatrix() {
  ...
  const openEditor = (itemId: string, locationId: string, currentQty: number) => {
    ...
    setEditValue(String(currentQty ?? 0))
    setEditError(null)
  }
              ...
              <Card key={item.id} padding="sm">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {sortedLocations.map(l => {
                    ...
                    const isEditing = editingKey === key
                    return (
                      <div key={l.id} className="flex items-center justify-between gap-1 rounded-lg border border-[var(--border-light)] px-2 py-1">
                        ...
                        {isEditing ? (
                          <input
                            ...
                            value={editValue}
                            onChange={e => setEditValue(e.target.value)}
                            onBlur={() => commitEditor(item.id, l.id)}
                            onKeyDown={e => {
                              if (e.key === 'Enter') commitEditor(item.id, l.id)
                              if (e.key === 'Escape') { setEditingKey(null); setEditError(null) }
                            }}
