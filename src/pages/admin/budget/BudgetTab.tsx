...
export function BudgetTab({ projectId, projectType }: Props) {
  ...
  const { data: tradesData = [], error: tradesError, refetch: tradesRefetch } = useQuery({
  ...
  if (tradesError) return (
    <div className="p-8 text-center">
      <p className="text-sm text-[var(--text-secondary)] mb-3">Unable to load budget data. Check your connection and try again.</p>
      <button onClick={() => tradesRefetch()} className="text-xs font-semibold text-[var(--navy)] border border-[var(--navy)] px-3 py-2 rounded-lg">Retry</button>
    </div>
  );
