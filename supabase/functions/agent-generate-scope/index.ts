...
serve(async (req) => {
  ...
  try {
    ...
    const { project_id, budget_quote_id, trade, additional_instructions } = body

    if (!project_id || !budget_quote_id || !trade) {
      return new Response(
        JSON.stringify({ error: 'project_id, budget_quote_id, and trade are required' }),
