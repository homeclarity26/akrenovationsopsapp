...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt =
      (basePrompt ??
        'You are a social media content writer for AK Renovations, a high-end residential remodeling contractor in Summit County, Ohio.') +
      `
...
No em dashes.`
    ...
    const { text: content, usage: _u } = await callClaude(
      systemPrompt,
      `Generate Instagram and Facebook posts for these ${scored.length} project photo(s)

    logAiUsage({ function_name: 'agent-social-content', model_provider: 'anthropic', model_name: 'claude-sonnet-4-20250514', input_tokens: _u.input_tokens, output_tokens: _u.output_tokens, duration_ms: Date.now() - _t0, status: 'success' }) from this week:
