...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt =
      (basePrompt ??
        buildSystemPrompt(company, 'customer relations assistant')) +
      `
...
Draft the review request message.`,
    ...
    logAiUsage({ function_name: 'agent-review-request', model_provider: 'anthropic', model_name: 'claude-sonnet-4-20250514', input_tokens: _u.input_tokens, output_tokens: _u.output_tokens, duration_ms: Date.now() - _t0, status: 'success' })
